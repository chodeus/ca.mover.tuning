...
- wip: RELEASING for the rework
- wip: urgent-fix test runs the full cycle
- wip: promotion and merge-back rework
- Give each release check its own function (ec83eef)
- Update copyright year and authorship in plugin files (d93162c)
- Update contributor acknowledgment in README.md (e125b8e)
- Apply the priorities and turbo write to the forced move again (94238a2)
- Give the forced move its own function, dispatched before the Mover Tuning checks (a482319)
- Apply the forced move's parity option to the forced move again (0972163)
- Correct the Chinese translation against en_US (e08f84e)
- Report blank texts and accept bare INI keyword keys, as Unraid does (4d468ee)
- Check placeholders against the key in every language file (5c38a22)
- Accept indented comments and check page texts holding a literal $ (848ae58)
- Keep age_mover and debug_mover executable after install (949ce14)
- Send a planned folder to Unraid move without its contents (1fa6b3a)
- Move a planned folder without the files added to it since (9d9c6b5)
- Read moveInUse from the global settings only (bd99594)
- Hold memory-mapped files at planning too (1a988ea)
- Hold open files from moves only, not from syncs (ca1ca97)
- Keep lastCacheSync when a move is held as in use (c09ca1b)
- Match the shorter in-use help (2134773)
- Shorten the help for the in-use setting (f1f8493)
- Leave files where they are when fuser is missing and moveInUse is no (c80d039)
- Split the language check into one function per step (4be41a3)
- Check the language files the way Unraid reads them (0cfba79)
- Fill in the rest of the Simplified Chinese translation (ccb2a24)
- Add a Simplified Chinese draft from the translation in issue #74 (5f2e093)
- Bring the German and Russian notice keys up to date with en_US (ef08814)
- Translate the in-use setting into German, French and Russian (d00ef24)
- Translate the rsync-error notice into German, French and Russian (8f38b5f)
- Declare the rsync-error counters beside the not-moved ones (6cabd54)
- Judge rsync errors once; block a staged chain on one and report files that arrived despite one (e5d05d1)
- Declare GONE_BEFORE beside HANDLED_LINKS (e1ce8b7)
- Treat names gone before the transfer as vanished; take vanished names off the remaining counters (5dac383)
- Count a group as moved once every name arrived or was gone; keep gone names out of rsync (ddda6a4)
- Pass a staged row's vanished names to its sync row when validation stops it (6f7f791)
- Keep hardlink groups on different disks apart; skip vanished names in filename validation (fc50608)
- Add a setting to move files that are in use, off by default (e952a61)
- Plan hardlink names together and report only real failures (0adfb33)
- Leave files a process has open where they are (0c3bbb8)
...
- new: Added localized threshold warnings and range-adjustment messages while preserving existing behavior.
- new: Added version-specific guidance for Unraid 7.2.1, including updated scheduler details and mover command usage.
- new: Notifications now follow the configured GUI language for configuration, validation, sync, cleanup, startup, and completion events.
- new: Dynamic notification details (filenames, options, elapsed time) are preserved, with English fallback when translations are missing.
- new: Added a localized "Mover Tuning Notification" label.
- fix: Transfer completion is now verified more reliably before reporting success.
- fix: Sync timestamps no longer update when transfers fail.
- fix: Hardlink groups are handled safely, preventing partial moves and validating all linked filenames.
- fix: Filenames with special characters, separators, quotes, or path-like text are now validated correctly.
- fix: Successful and failed transfers are tracked separately, with accurate counts and clear failure details.
- fix: Mover reporting now distinguishes successful, skipped, incomplete, vanished, and failed items.
- fix: Post-operation verification ensures moves and syncs are only reported as successful after destination arrival and source removal.
- fix: Remaining-work tracking is now based on verified results.
- fix: Folder-only move and sync plans are processed correctly.
- fix: Non-numbered, disk-prefixed storage paths remain eligible for processing.
- fix: Improved skip-list handling, file-type filtering, and numbered array-disk detection.
- fix: Sync status no longer advances after unsuccessful transfers.
- fix: Notifications and logs now show separate totals for successful and unsuccessful items.
- fix: Move and sync reporting now includes verified folders and transferred size.
- doc: Reorganized inline help for test mode, validation, filtering, options, logging, and support links.
- doc: Improved wording, punctuation, terminology, and formatting throughout the plugin's help content.
...
- new: Version detection and conditional logic — core files and UI elements gated based on Unraid version.
